...
function App() {
  ...
  return (
    ...
    <div className="d">
      ...
      {Items.slice(Math.min(Math.max(lazy-24,0)),Math.min(lazy+24,Items.length-1)).map((item,index)=><Video inView channel={item.channel} title={item.title} time={item.time} url={item.url} keys={item.id} key={item.id} Lazy={setLazy} Wait={setWait} wait={wait}></Video>)}
